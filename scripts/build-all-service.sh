...
(cd ./solutions/IdentitySolution/DuendeIdentityServer && dotnet restore --packages "$project_root/data/nuget" --verbosity normal)
cd "$project_root"

echo -e "\e[95mRestoring Nuget Package in conversation service ....\e[0m"
(cd ./solutions/ConversationSolution/src/ConversationService.API && dotnet restore --packages "$project_root/data/nuget" --verbosity normal)
cd "$project_root"

echo -e "\e[95mRestoring Nuget Package in chat hub service ....\e[0m"
(cd ./solutions/ChatHubSolution/src/ChatHub && dotnet restore --packages "$project_root/data/nuget" --verbosity normal)
cd "$project_root"
...
echo -e "\e[95mRestoring Nuget Package in upload file service ....\e[0m"
(cd ./solutions/NotificationSolution/src/NotificationService.API && dotnet restore --packages "$project_root/data/nuget" --verbosity normal)
cd "$project_root"
